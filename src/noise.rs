...
impl ColoredNoise{
    pub fn new(beta: f64, length: usize, sigma: f64)->ColoredNoise{
        ...
        let mut norm=0.0;
        let kernel_f:Vec<_>=(0..length/2+1).map(|i|{
            let x=if i==0{
        ...
        let mut psd:Vec<_>=kernel_f.iter().map(|&x|{
            x.re.powi(2)
        }).collect();

        let mut psd_complex:Vec<Complex64>=psd.iter().map(|&x| x.into()).collect();
        ...
        let buffer:std::collections::VecDeque<_>=(0..length).map(|_|{
            normal_rand.sample(&mut rng)
        }).collect();
        ColoredNoise{
            normal_rand, 
            rng, 
    ...
    pub fn convolve(&self)->f64{
        self.buffer.iter().zip(self.kernel.iter()).map(|(&x, &y)|{x*y}).sum()
    }
    ...
    pub fn truncated_autocorr(&self, length: usize)-> Vec<f64>{
        ...
        for i in 0..length/2{
            result[i]=self.autocorr[i];
        }
        for i in length/2..length{
            result[i]=self.autocorr[self.autocorr.len()-length+i];
        }
        result
    }
}