...
pub fn gal2eq_ring(data_in: &[f64]) -> Vec<f64> {
    ...
    for (i, r) in result.iter_mut().enumerate().take(data_in.len()) {
        ...
        *r = natural_interp_ring(nside, data_in, ptr_gal);
    }
    result
}
...
pub fn eq2gal_ring(data_in: &[f64]) -> Vec<f64> {
    ...
    for (i, r) in result.iter_mut().enumerate().take(data_in.len()) {
        ...
        *r = natural_interp_ring(nside, data_in, ptr_eqp);
    }
    result
}
...
pub fn gen_tod(data: &[f64], ra: &[f64], dec: &[f64]) -> Vec<f64> {
    ...
    ra.iter()
        ...
        .map(|(&r, &d)| {
            ...
            natural_interp_ring(nside, data, ptg)
        })
        .collect()
}
